...
function parseVoiceCommand(text: string): ParsedCommand {
  ...
  // ── "Berapa temperatur / suhu"
  if (/berapa\s*(temperatur|suhu|temperature|panas)|suhu\s*(sekarang|saat ini|berapa)/.test(t)) {
    return { type: "sensor_temp" };
  }

  // ── "Berapa kelembapan / kelembaban"
  if (/berapa\s*(kelembap|kelembab|humidity|lembab)|kelembap(an)?\s*(sekarang|berapa)/.test(t)) {
    return { type: "sensor_hum" };
  }

  // ── "Cek sensor / suhu" (umum)
  if (/cek\s*(suhu|sensor|temperature|kelembab|kelembap)|sensor\s*(suhu|data)/.test(t)) {
    return { type: "sensor" };
  }

  // ── "Nyalakan Variasi 1"
  if (/(nyalakan|aktifkan|jalankan|hidupkan|mulai)\s*(variasi|mode)\s*(1|satu|pertama)/.test(t) ||
      /variasi\s*(1|satu)\s*(on|nyala|aktif)/.test(t)) {
    return { type: "variasi", variasiId: 1, state: true };
  }

  // ── "Nyalakan Variasi 2"
  if (/(nyalakan|aktifkan|jalankan|hidupkan|mulai)\s*(variasi|mode)\s*(2|dua|kedua)/.test(t) ||
      /variasi\s*(2|dua)\s*(on|nyala|aktif)/.test(t)) {
    return { type: "variasi", variasiId: 2, state: true };
  }

  // ── "Matikan / Stop Variasi"
  if (/(matikan|hentikan|stop|off)\s*(variasi|semua\s*variasi)|variasi\s*(off|mati|stop)/.test(t)) {
    return { type: "variasi", variasiId: 0, state: false };
  }

  // ── "Matikan semua"
  if (/semua\s*(mati|matikan|off)|matikan\s*semua|(padamkan|nonaktifkan)\s*semua/.test(t)) {
    return { type: "all", state: false };
  }

  // ── "Nyalakan semua"
  if (/semua\s*(nyala|nyalakan|on)|nyalakan\s*semua|(hidupkan|aktifkan)\s*semua/.test(t)) {
    return { type: "all", state: true };
  }

  // ── "Nyalakan lampu" (tanpa nomor → semua)
  if (/^(nyalakan|hidupkan|aktifkan)\s*lampu$/.test(t) || /^lampu\s*(on|nyala|hidup)$/.test(t)) {
    return { type: "all", state: true };
  }

  // ── "Matikan lampu" (tanpa nomor → semua)
  if (/^(matikan|padamkan|nonaktifkan)\s*lampu$/.test(t) || /^lampu\s*(off|mati|padam)$/.test(t)) {
    return { type: "all", state: false };
  }
  ...
  for (const [word, id] of Object.entries(relayMap)) {
    const onPat = new RegExp(
      `(nyala|nyalakan|on|hidupkan|aktifkan).{0,10}lampu.{0,6}${word}` +
      `|lampu.{0,6}${word}.{0,10}(nyala|nyalakan|on|hidup|aktif)`
    );
    const offPat = new RegExp(
      `(mati|matikan|off|padamkan|nonaktifkan).{0,10}lampu.{0,6}${word}` +
      `|lampu.{0,6}${word}.{0,10}(mati|matikan|off|padam|nonaktif)`
    );
  ...
  useEffect(() => {
    ...
    recognition.onresult = (e: any) => {
      ...
      if (e.results[e.results.length - 1].isFinal) {
        ...
        } else if (cmd.type === "sensor_temp") {
          resultMsg = `✓ Suhu saat ini: ${sensor.temp !== null ? sensor.temp + "°C" : "data belum tersedia"}`;
          ok = true;
        } else if (cmd.type === "sensor_hum") {
          resultMsg = `✓ Kelembapan saat ini: ${sensor.hum !== null ? sensor.hum + "%" : "data belum tersedia"}`;
          ok = true;
        } else if (cmd.type === "sensor") {
          resultMsg = `✓ Suhu: ${sensor.temp ?? "--"}°C | Kelembapan: ${sensor.hum ?? "--"}%`;
          ok = true;
        } else if (cmd.type === "variasi") {
          setVariasiCmd(cmd.variasiId!);
          if (cmd.variasiId === 0) {
            resultMsg = `✓ Variasi dihentikan`;
          } else {
            resultMsg = `✓ Variasi ${cmd.variasiId} diaktifkan`;
          }
          ok = true;
          notify(
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className={`border rounded-2xl p-5 transition-all ${
          ...
          <div className="mb-3 space-y-2">
            {[
              { label: "Kontrol Lampu", cmds: ["Nyalakan lampu", "Matikan lampu", "Nyalakan lampu satu", "Matikan lampu dua", "Nyalakan semua", "Matikan semua"] },
              ...
              <div key={group.label}>
                <p className="text-[10px] text-slate-600 uppercase tracking-wider mb-1">{group.label}</p>
